...
const VehicleHgsLoadings = {
  ...
  async getByVehicleId(vehicleId) { 
    const { rows } = await db.query('SELECT * FROM vehicle_hgs_loadings WHERE vehicle_id = $1 ORDER BY loading_date DESC', [vehicleId]);
    return rows;
  },
  async create(data) {
  ...
  async updateHgsLoading(id, data) {
  ...
  async deleteHgsLoading(id) { return db.query('DELETE FROM vehicle_hgs_loadings WHERE id = $1', [id]); }
};
