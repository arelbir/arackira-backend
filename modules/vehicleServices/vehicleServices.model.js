...
class VehicleServices {
  static async getAll() { 
    const { rows } = await db.query('SELECT * FROM vehicle_services'); 
    return rows;
  }
  
  static async getById(id) { 
    const { rows } = await db.query('SELECT * FROM vehicle_services WHERE id = $1', [id]); 
    return rows[0];
  }

  static async getByVehicleId(vehicleId) { 
    const { rows } = await db.query('SELECT * FROM vehicle_services WHERE vehicle_id = $1 ORDER BY service_date DESC', [vehicleId]);
    return rows;
  }

  static async create(data) {
    const { rows } = await db.query(`INSERT INTO vehicle_services (
  ...
  static async update(id, data) {
    const { rows } = await db.query(`UPDATE vehicle_services SET
  ...
  static async delete(id) { 
    const { rows } = await db.query('DELETE FROM vehicle_services WHERE id = $1 RETURNING *', [id]); 
    return rows[0];
  }
}
